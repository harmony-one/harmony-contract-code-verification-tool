...
// const byteCodeEnd600 = "a265627a7a7";
...
const trimByteCode = (solidityVersion: string) => (byteCode: string) => {
  ...
  try {
    ...
    if (
      solidityMinorVersion <= 4 ||
      (solidityMinorVersion >= 4 && solidityPatchVersion <= 22)
    ) {
    ...
    // if (solidityMinorVersion >= 6) {
    //   byteCodeEnd = byteCodeEnd600;
    // }
